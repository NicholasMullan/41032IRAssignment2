...
%% Create the robot model
        function CreateModel(self)   
            ...
            link(2) = Link([0     0.12  0       0   0]);
            link(3) = Link([pi      0.19  0       pi/2   0]);
            % link(4) = Link([0      0.1357  0.425   -pi     0]);
            % link(5) = Link([0      0.1197  0.39243 pi      0]);
            % link(6) = Link([0      0.093   0       -pi/2   0]);
            % link(7) = Link([0      0.093   0       -pi/2	0]);
            % link(8) = Link([0      0       0       0       0]);
            
            % Incorporate joint limits
            link(1).qlim = [-0.8 -0.01];
            link(2).qlim = [0,0]*pi/180;
            link(3).qlim = [-360 360]*pi/180;
            % link(4).qlim = [-150 150]*pi/180;
            % link(5).qlim = [-3.5 300]*pi/180;
            % link(6).qlim = [-360 360]*pi/180;
            % link(7).qlim = [-124 124]*pi/180;
