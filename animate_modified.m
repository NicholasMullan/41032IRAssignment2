...
function animate(robot, qq)
    ...
    while true
            ...
            for handle=handles'
                ...
                if robot.mdh
                    ...
                    for L=1:N
                        ...
                        if robot.links(L).isprismatic()
                            %disp(['q(L) = ', num2str(q(L))])
                            % scale the box representing the prismatic joint
                            % it is based at the origin and extends in z-direction
                            
                            % Looking at the way this pjoint issue was
                            % solved in the V9 code, the error was simply
                            % commented out.
                            scicEdit = true;
        ...
        try 
            ...
            warning(['in animate(). It is likely that there is no robot plot handle named: ''',robot.name,''' in any open figures. Has it been deleted?'])
            break;
        end
    end
